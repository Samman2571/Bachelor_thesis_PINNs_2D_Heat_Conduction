...
# Define the PINN model (same as before)
...
def boundary_loss(model, x, y, t, length, boundary_temp):
    u = model(x, y, t)
    
    x_boundary = (x <= 1e-6) | (x >= length - 1e-6)
    y_boundary = (y <= 1e-6) | (y >= length - 1e-6)
    
    loss = torch.mean((u[x_boundary] - boundary_temp)**2) + torch.mean((u[y_boundary] - boundary_temp)**2)
...
learning_rate = 0.001
epsilon = 0.1
boundary_temp = 25.0  # Fixed boundary temperature
...
for epoch in range(epochs):
    ...
    loss_boundary = boundary_loss(model, x, y, t, length, boundary_temp)
...
anim.save('heat_conduction_center_source_dirichlet.gif', writer='pillow', fps=10)
plt.close(fig)

print("Simulation complete. GIF saved as 'heat_conduction_center_source_dirichlet.gif'")