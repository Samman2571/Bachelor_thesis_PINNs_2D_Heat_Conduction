...
layers = [32, 32, 32, 32]
alpha = 0.1  # Reduced thermal diffusivity to slow down the process
learning_rate = 1e-4
num_epochs = 20000
...
t = torch.rand((num_points, 1), requires_grad=True)  # t in [0, 1]
...
t_bc = torch.rand((num_bc_points, 1)).requires_grad_(True)
...
# Saving the model
model_path = 'pinn_heat_redistribution_neumann.pth'
...
save_path = r'C:\Users\samma\OneDrive\Desktop\Thesis_Samman_Aryal\V5\V6\FInals\Images\heat_redistribution_neumann'
...
# Create GIF
gif_path = os.path.join(save_path, 'heat_redistribution_neumann.gif')
